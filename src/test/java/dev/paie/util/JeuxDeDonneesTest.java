...
public class JeuxDeDonneesTest {
	...
	public void test_cotisationsNonImposables() {
		...
		Stream.of("EP01", "EP02", "EP03", "EP04", "EP05", "EP06", "EP07", "EP12", "EP19", "EP20", "EPR1", "E900",
				"EP28", "EP37")
				.forEach(code -> assertThat(cotisationsNonImposables.stream().filter(c -> c.getCode().equals(code)).findAny().isPresent()).as("verification code " + code).isTrue());
	}
	...
	public void test_cotisationImposables() {
		...
		Stream.of("SP01", "SP02").forEach(code -> assertThat(cotisationsImposables.stream().filter(c -> c.getCode().equals(code)).findAny().isPresent()).as("verification code " + code).isTrue());
	}
